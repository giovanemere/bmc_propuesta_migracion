...
DiagramGenerator = RefinedDiagramGenerator
    ...
    def generate_all_refined(self, project_name: str = "BMC") -> Dict[str, str]:
        ...
        # Generar diagramas unificados desde MCP
        from .mcp_generator import UnifiedMCPGenerator
        unified_generator = UnifiedMCPGenerator(self.config, temp_output_dir)
        unified_results = unified_generator.generate_all_diagrams(project_name)
        
        # Agregar resultados unificados
        for diagram_type, diagram_path in unified_results.items():
            results[f"unified_{diagram_type}"] = diagram_path
        
        # Archivos ya organizados en estructura MCP
        ...
        from .doc_generator import ImplementationDocGenerator
        doc_generator = ImplementationDocGenerator(self.config, temp_output_dir)
        doc_results = doc_generator.generate_implementation_docs(project_name)
        
        # Agregar prompts y documentación a resultados
        for prompt_type, prompt_path in prompt_results.items():
            results[f"prompts_{prompt_type}"] = prompt_path
            
        for doc_type, doc_path in doc_results.items():
            results[f"docs_{doc_type}"] = doc_path
        
        # Archivos ya están en estructura MCP final
        print("✓ MCP prompts generated")
        print("✓ Implementation documentation generated")
        print("✓ Unified diagrams (Mermaid + DrawIO) generated")
